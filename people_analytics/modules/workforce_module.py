# modules/workforce_module.py — v2.9 | Executive
...
def run_workforce_module():
    ...
    if "ManagerID" in df.columns:
        ...
        st.dataframe(manager_counts, use_container_width=True)
    ...
    if span_df is not None:
        data_blocks.append({"title":"Manager Spans","desc":"Direct reports per manager","df":span_df,"fig":fig_span,"insights":[f"Average span: {span_df['DirectReports'].mean():.1f}"]})
