# ============================================
# modules/consolidated_module.py — v6.0 | HR Leadership Deck Generator (Executive Stable)
# ============================================
...
# ✅ Fix: avoid bool() conversion ambiguity
...
try:
    ...
    st.warning(f"⚠️ Workforce data issue: {e}")
    work_blocks = []
...
try:
    ...
    st.warning(f"⚠️ Performance data issue: {e}")
    perf_blocks = []
...
try:
    ...
    st.warning(f"⚠️ Engagement data issue: {e}")
    eng_blocks = []
...
try:
    if "CTC" in comp_df.columns:
        ...
        # ✅ Fixed Benchmark Comparison (no crash)
        if bench_df is not None and {"JobLevel", "MarketMedianCTC"}.issubset(bench_df.columns):
            ...
            comp_blocks.append({"title": "Market Benchmark Comparison", "desc": "Internal pay vs market medians", "df": _round_df(market_summary), "fig": fig_market})
    else:
        comp_blocks = []
except Exception as e:
    st.warning(f"⚠️ Compensation data issue: {e}")
    comp_blocks = []
...
try:
    if "AttritionFlag" in attr_df.columns:
        ...
        attr_blocks = [
            {"title": "Attrition Overview", "desc": "Overall attrition metrics", "df": pd.DataFrame({"Attrition %": [round(attr_rate,2)], "Avg Tenure (mo)": [round(avg_tenure,2) if avg_tenure else "N/A"]}), "fig": None},
            {"title": "Departmental Attrition", "desc": "Attrition % by Department", "df": _round_df(dept), "fig": fig_attr},
        ]
    else:
        attr_blocks = []
except Exception as e:
    st.warning(f"⚠️ Attrition data issue: {e}")
    attr_blocks = []
