# ============================================
# modules/consolidated_module.py — v6.2 | HR Leadership Deck Generator (Stable Core)
# ============================================
...
if "CTC" in comp_df.columns:
    ...
    # 🧠 Safe benchmark merge (fixes blank screen)
    try:
        if bench_df is not None and {"JobLevel", "MarketMedianCTC"}.issubset(bench_df.columns):
            ...
            bench_df = bench_df.dropna(subset=["JobLevel", "MarketMedianCTC"])
            bench_df = bench_df[bench_df["MarketMedianCTC"] > 0]

            merged = comp_df.merge(
                bench_df[["JobLevel", "MarketMedianCTC"]].drop_duplicates(),
            ...
            market_summary = merged.groupby("JobLevel", observed=True)[["CTC", "MarketMedianCTC", "DiffPct"]] \
            ...
            if not market_summary.empty:
                ...
                st.warning("⚠️ Benchmark data uploaded but contains no valid values.")
        elif bench_df is not None:
            st.warning("⚠️ Benchmark file missing required columns: JobLevel, MarketMedianCTC")
    except Exception as e:
        st.error(f"⚠️ Benchmark comparison skipped due to data error: {e}")
